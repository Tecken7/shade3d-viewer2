...
function PreloadIcons() {
  useEffect(() => {
    try { Object.values(ICONS).forEach((src) => { const img = new Image(); img.decoding = "async"; img.src = src }) } catch {}
  }, [])
  return null
}
...
function autoSmoothGeometry(geometry, angleDeg = DEFAULT_SMOOTH_ANGLE) {
  ...
  const vCount = pos.count, triCount = vCount / 3
  ...
  const tmp = new THREE.Vector3(), cosThresh = Math.cos(angleRad)
  groups.forEach((cornerIndices) => {
    ...
    for (let idx = 0; idx < cornerIndices.length; idx++) {
      const ci = cornerIndices[idx], nRef = localFaceNs[idx]
      let nx = 0, ny = 0, nz = 0
...
function AnyModel({
  ...
  keepMaterials = false,
  wireframe = false,            // přepínač pro overlay hran
  edgesAngle = 30,              // prahový úhel pro EdgesGeometry (deg)
}) {
  ...
  // vytvoření/obnova overlay čar pro daný mesh
  const rebuildEdgesForMesh = (mesh) => {
    // zlikviduj staré overlay
    ...
    try {
      ...
      if (!geom) return
      const eGeom = new THREE.EdgesGeometry(geom, edgesAngle) // thresholdAngle ve stupních
      ...
      lines.renderOrder = (mesh.renderOrder || 0) + 10 // nad mesh
      mesh.add(lines)
      mesh.userData._edges = lines
    } catch (e) { console.warn("Edges overlay failed:", e) }
  }

  // procházej strom a pro každý mesh zavolej cb
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!cancelled) {
          // přidej overlay hrany podle stavu
          forEachMesh(obj, (mesh) => rebuildEdgesForMesh(mesh))
          setObject3D(obj)
          setLoading(false)
  ...
  // Re-smoothing – regenerace geometrie i overlay hran
  useEffect(() => {
    if (!object3D) return
    forEachMesh(object3D, (child) => {
      ...
      // po změně geometrie znova vybudovat edges overlay
      rebuildEdgesForMesh(child)
    })
  }, [object3D, autoSmooth, edgesAngle])

  // Materiály + overlay aktualizace při změně vlastností
  useEffect(() => {
    if (!object3D) return
    forEachMesh(object3D, (child) => {
      if (keepMaterials) {
      ...
      // jen přepnout viditelnost overlay podle wireframe
      if (child.userData._edges) child.userData._edges.visible = !!wireframe
      else if (wireframe) rebuildEdgesForMesh(child)
    })
...
function TouchTrackballControls({ target = [0, 0, 0] }) {
  ...
  useEffect(() => {
    ...
    controls.rotateSpeed = 5.0; controls.zoomSpeed = 1.2; controls.panSpeed = 1.0
    controls.staticMoving = true; controls.dynamicDampingFactor = 0.15
    controls.mouseButtons = { LEFT: THREE.MOUSE.ROTATE, MIDDLE: THREE.MOUSE.ZOOM }
    controlsRef.current = controls
    return () => { controls.dispose() }
  }, [camera, gl])
  useEffect(() => { const c = controlsRef.current; if (!c) return; c.target.set(target[0], target[1], target[2]); c.update() }, [target])
  useFrame(() => { const c = controlsRef.current; if (!c) return; if (camera.isOrthographicCamera) c.panSpeed = camera.zoom * 0.4; c.update() })
...
function RightButtonPan({ setTarget }) {
  ...
  const right = new THREE.Vector3(), up = new THREE.Vector3(), deltaWorld = new THREE.Vector3()
  useEffect(() => {
    ...
    const onDown = (e) => {
      ...
      isPanning.current = true; last.current = { x: e.clientX, y: e.clientY }
      pointerIdRef.current = e.pointerId; try { el.setPointerCapture?.(e.pointerId) } catch {}
    }
    const onMove = (e) => {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <button
        ...
        onClick={() => onChange(!checked)} onKeyDown={handleKey}
        style={{
          position: "relative", width: TRACK_W, height: TRACK_H, borderRadius: 999,
    ...
    ;(async () => {
      try {
        ...
        if (manifestUrlParam) {
          ...
          const logoUrl = m?.logo?.url || DEFAULT_LOGO
          setLogoCfg({ url: logoUrl || null, opacity: clamp01(parseFloat(getParam("logoOpacity") ?? "0.9")), width: parseInt(getParam("logoWidth") ?? (window.innerWidth < 768 ? "120" : "160"), 10), pos: getParam("logoPos") || "bc" })
          const qOn = getParam("headlight"), qI = parseFloat(getParam("headlightI") ?? "NaN")
        ...
        if (f) {
          ...
          setLogoCfg({ url: getParam("logo") === "none" ? null : getParam("logo") || DEFAULT_LOGO, opacity: clamp01(parseFloat(getParam("logoOpacity") ?? "0.9")), width: parseInt(getParam("logoWidth") ?? (window.innerWidth < 768 ? "120" : "160"), 10), pos: getParam("logoPos") || "bc" })
          setPhotos([])
          return
        }
  ...
  return (
    <div className="stage" style={{ position: "relative", width: "100vw", height: "100vh", background: "black" }}>
      ...
      <Canvas orthographic camera={{ position: [0, 0, 100], near: 0.01, far: 100000 }} gl={{ alpha: true }} onCreated={({ gl }) => gl.setClearAlpha(0)} style={{ position: "absolute", inset: 0, zIndex: 1, background: "transparent" }}>
        <>
          <ambientLight intensity={1 * 0.4 * (headlightCfg.enabled ? 0.5 : 1)} />
          <directionalLight position={[0, 5, 5]} intensity={1 * 1.5 * (headlightCfg.enabled ? 0.5 : 1)} />
          <directionalLight position={[-10, 0, 0]} intensity={1 * 1.0 * (headlightCfg.enabled ? 0.5 : 1)} />
          <directionalLight position={[10, 0, 0]} intensity={1 * 1.2 * (headlightCfg.enabled ? 0.5 : 1)} />
          <directionalLight position={[0, -5, -5]} intensity={1 * 0.8 * (headlightCfg.enabled ? 0.5 : 1)} />
...
function AutoCenterAndFrame({
  ...
  useEffect(() => {
    ...
    const centerAll = new THREE.Vector3(), dims = new THREE.Vector3()
    boxAll.getCenter(centerAll); boxAll.getSize(dims)
    if (centerMode === "per") {
      root.children.forEach((child) => {
        const b = new THREE.Box3().setFromObject(child); if (b.isEmpty()) return
        const cWorld = new THREE.Vector3(); b.getCenter(cWorld); child.position.sub(cWorld)
      })
    ...
    camera.near = Math.max(0.01, safeDist * 0.001); camera.far = safeDist * 50 + 100
    camera.position.set(ctr.x, ctr.y, ctr.z + safeDist); camera.zoom = Math.max(newZoom, 0.01)
    camera.updateProjectionMatrix()
