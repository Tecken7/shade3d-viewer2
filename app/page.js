...
/* ---------- Helpers: query → dynamické soubory ---------- */
...
function parseFilesFromQuery() {
  ...
  // Kompatibilita se starým režimem:
  ...
  return (
    <div style={{ width: '100vw', height: '100vh' }}>
      ...
      <div
        ...
        style={{
          position: 'absolute', top: 10, left: 10, zIndex: 1, color: 'white',
      ...
      >
        {files.map((f, i) => (
          <div className="control-row" key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, margin: '6px 0' }}>
            ...
            >
              <img src={ICONS.eye}    alt="" className="icon-img icon-on"  width="20" height="20" style={{ position:'absolute', inset:0, width:20, height:20, margin:'auto', opacity:1 }} />
              <img src={ICONS.eyeOff} alt="" className="icon-img icon-off" width="20" height="20" style={{ position:'absolute', inset:0, width:20, height:20, margin:'auto', opacity:0 }} />
            </button>
          </div>
        ))}
        ...
        >
          <span className="arrow-stack" aria-hidden style={{ position: 'relative', width: 16, height: 16, display: 'inline-block' }}>
            <img src={ICONS.arrowClosed} className="arrow-img arrow-closed" width="16" height="16" style={{ position:'absolute', left:0, top:0, width:16, height:16, opacity: showLights ? 0 : 1 }} loading="eager" decoding="async" alt="" />
            <img src={ICONS.arrowOpen}   className="arrow-img arrow-open"   width="16" height="16" style={{ position:'absolute', left:0, top:0, width:16, height:16, opacity: showLights ? 1 : 0 }} loading="eager" decoding="async" alt="" />
          </span>
      ...
      {/* Zachovávám tvoje globální styly; jen rozšiřuju .row-label */}
      <style jsx global>{`
        .slider {
          appearance: none; width: var(--slider-width, 180px); height: 14px; background: transparent; margin: 5px 0; display: inline-block;
        }
